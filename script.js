...
async function runSweep(keywords) {
  ...
  for (const kw of keywords) {
    ...
    try {
      ...
      state.scannedCount += groups.length;

      let hits = 0;
      for (const g of groups) {
        if (state.stopRequested) break;
        const isOwnerless = g.owner === null || g.owner === undefined;
      ...
      logLine(`&nbsp;&nbsp;→ checked ${groups.length} groups, <span class="tag">${hits} ownerless</span> found`);
    } catch (err) {
...
els.btnCheck.addEventListener('click', async () => {
  ...
  try {
    ...
    const ownerless = g.owner === null || g.owner === undefined;
